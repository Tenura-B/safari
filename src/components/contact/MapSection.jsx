...
const MapSection = () => {
  // Coordinates for the heart of Yala National Park area
  const center = { lat: 6.3815, lng: 81.3323 };
  const zoom = 11;
    ...
    <section id="map-location" className="bg-black py-20 px-6 md:px-20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          ...
          <a 
            href="https://www.google.com/maps/search/?api=1&query=6.3815,81.3323" 
            target="_blank" 
        ...
        <div className="h-[500px] w-full rounded-xl overflow-hidden border-[12px] border-white shadow-2xl relative">
          <GoogleMapReact
            ...
            options={mapOptions}
          >
            <Marker lat={6.3815} lng={81.3323} />
          </GoogleMapReact>

          {/* Floating Label overlay */}
          <div className="absolute top-6 left-6 bg-white/90 backdrop-blur-sm p-3 px-5 rounded shadow-md hidden md:block">
            <p className="text-[10px] font-black uppercase tracking-tighter text-zinc-400">Main Entrance</p>
            <p className="text-xs font-bold uppercase italic text-black">Yala National Park</p>
          </div>
        </div>
      </div>
    </section>
